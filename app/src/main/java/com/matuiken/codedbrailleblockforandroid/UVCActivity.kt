 ...
 */
package com.matuiken.codedbrailleblockforandroid
...
import kotlinx.android.synthetic.main.activity_uvc.*

class UVCActivity : BaseActivity(), CameraDialogParent {
    /**
     * for accessing USB
     */
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        uvc_camera_button.setOnCheckedChangeListener(mOnCheckedChangeListener)
        mUSBMonitor = USBMonitor(this, mOnDeviceConnectListener)
        mCameraHandler = UVCCameraHandlerMultiSurface.createHandler(this, uvc_camera_view,
    ...
    private val mOnCheckedChangeListener = CompoundButton.OnCheckedChangeListener { compoundButton, isChecked ->
        when (compoundButton.id) {
            R.id.uvc_camera_button -> if (isChecked && !mCameraHandler!!.isOpened) {
                CameraDialog.showDialog(this@UVCActivity)
            } else {
                stopPreview()
            }
        }
    ...
    private fun setCameraButton(isOn: Boolean) {
        ...
        runOnUiThread({
            try {
                uvc_camera_button.setOnCheckedChangeListener(null)
                uvc_camera_button.isChecked = isOn
            } finally {
                uvc_camera_button.setOnCheckedChangeListener(mOnCheckedChangeListener)
            }
        }, 0)
    ...
    private fun startPreview() {
        ...
        uvc_camera_view!!.resetFps()
        mCameraHandler!!.startPreview()
        runOnUiThread {
            try {
                val st = uvc_camera_view!!.surfaceTexture
                if (st != null) {
    ...
    private val mOnDeviceConnectListener: OnDeviceConnectListener = object : OnDeviceConnectListener {
        override fun onAttach(device: UsbDevice?) {
            Toast.makeText(this@UVCActivity,
